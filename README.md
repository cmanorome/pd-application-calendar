...
`POST /api/calendar/subscribe` stores the form answers and returns a live feed URL. `GET /c/{id}.ics` rebuilds that calendar so a phone can keep it up to date. Feeds expire after 13 months. Locally this saves files under `data/subscribed_plans/`. On Vercel, connect **Upstash Redis** (Storage tab) so the env vars `KV_REST_API_URL` and `KV_REST_API_TOKEN` (or `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`) are set, then redeploy.
